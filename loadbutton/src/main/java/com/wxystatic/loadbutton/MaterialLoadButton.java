package com.wxystatic.loadbutton;
...
public class MaterialLoadButton extends View {
    ...
    public MaterialLoadButton(Context context) {
        super(context);
        init(context,null);
    }

    public MaterialLoadButton(Context context, AttributeSet attrs) {
        super(context,attrs);
        init(context,attrs);
    }

    public MaterialLoadButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context,AttributeSet attrs){
        ...
        if(attrs != null){
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MaterialLoadButton, 0, 0);
            mColorPrimary = ta.getInt(R.styleable.MaterialLoadButton_load_btnNormalColor,Color.BLUE);
            mDisabledBgColor = ta.getColor(R.styleable.MaterialLoadButton_load_btnEnabledColor,Color.LTGRAY);
            mDisabledTextColor = ta.getColor(R.styleable.MaterialLoadButton_load_EnabledTextColor,Color.DKGRAY);
            String text = ta.getString(R.styleable.MaterialLoadButton_load_btnText);
            mText = text == null ? "" : text;
            mTextColor = ta.getColor(R.styleable.MaterialLoadButton_load_textColor,Color.WHITE);
            resetAfterFailed = ta.getBoolean(R.styleable.MaterialLoadButton_load_resetAfterFailed,true);
            mRippleColor = ta.getColor(R.styleable.MaterialLoadButton_load_btnRippleColor,Color.BLACK);
            mRippleAlpha = ta.getFloat(R.styleable.MaterialLoadButton_load_btnRippleAlpha,0.3f);
            mButtonCorner=ta.getDimension(R.styleable.MaterialLoadButton_load_btnRadius,0);
            loadWidth=ta.getDimension(R.styleable.MaterialLoadButton_load_loadWidth,0);
        ...
        mStrokePaint.setColor(mLoadColor);
        mStrokePaint.setStyle(Paint.Style.STROKE);
        mStrokePaint.setStrokeWidth(loadWidth);
        ...
        mPathEffectPaint.setColor(mLoadSuccessColor);
        mPathEffectPaint.setStyle(Paint.Style.STROKE);
        mPathEffectPaint.setStrokeWidth(loadWidth);

        mPathEffectPaint2 = new Paint();
        mPathEffectPaint2.setAntiAlias(true);
        mPathEffectPaint2.setColor(mLoadFailedColor);
        mPathEffectPaint2.setStyle(Paint.Style.STROKE);
        mPathEffectPaint2.setStrokeWidth(loadWidth);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_UP:
                if (Math.abs(event.getX()-mTouchX)<=20&&Math.abs(event.getY()-mTouchY)<=20){
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (loadWidth==0){
        mStrokePaint.setStrokeWidth((float) (getHeight()/2/5));
        mPathEffectPaint.setStrokeWidth((float) (getHeight()/2/5.5));
        mPathEffectPaint2.setStrokeWidth((float) (getHeight()/2/5.5));}
        mStrokePaint.setColor(mLoadColor);
        switch (mCurrentState) {
            ...
            case STATE_ANIMATION_SUCCESS:
                ...
                mStrokePaint.setColor(mLoadSuccessColor);
                canvas.drawCircle(width/2,height/2,mRadius-mDensity,mStrokePaint);
                break;
            case STATE_ANIMATION_FAILED:
                mStrokePaint.setColor(mLoadFailedColor);
                canvas.drawPath(mFailedPath,mPathEffectPaint2);
    ...
    private void playRippleAnimation(final boolean isTouchDown, final boolean isClick){
        setShadowDepth2();
